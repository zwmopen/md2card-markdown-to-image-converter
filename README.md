...
Cloudflare Remote MCP 网关和真实 Playwright 渲染后端也已经建立。MCP 可以通过约定接口提交单篇或批量任务，渲染服务会启动 Chromium，按真实 DOM 排版生成 PNG、JPEG、WebP 和 ZIP，并返回任务状态与下载地址。单节点部署已经支持任务元数据与结果持久化、重启恢复和过期清理；横向扩展仍需数据库、共享队列和对象存储。
...
详细运行、持久化目录、Docker 和 API 示例见 [`apps/render-service/README.md`](apps/render-service/README.md)。
